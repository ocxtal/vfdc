...
void khz_Start(void) 
{
    ...
    khz_CLKEN |= khz_CLKEN_MASK;
	khz_CLKSTBY |= khz_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: khz_Stop
********************************************************************************
*
* Summary:
...
void khz_Stop(void) 
{
    ...
    khz_CLKEN &= (uint8)(~khz_CLKEN_MASK);
	khz_CLKSTBY &= (uint8)(~khz_CLKSTBY_MASK);
}
...
{
    if ((khz_CLKEN & khz_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = khz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = khz_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(khz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(khz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: khz_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: khz_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = khz_GetSourceRegister();
    uint16 oldDivider = khz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = khz_CLKEN & khz_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((khz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: khz_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(khz_DIV_PTR);
}


/*******************************************************************************
* Function Name: khz_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    khz_MOD_SRC |= modeBitMask & (uint8)khz_MODE_MASK;
}


/*******************************************************************************
* Function Name: khz_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    khz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(khz_MODE_MASK));
}


/*******************************************************************************
* Function Name: khz_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return khz_MOD_SRC & (uint8)(khz_MODE_MASK);
}


/*******************************************************************************
* Function Name: khz_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = khz_GetDividerRegister();
    uint8 oldSrc = khz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        khz_MOD_SRC |= CYCLK_SSS;
        khz_MOD_SRC =
            (khz_MOD_SRC & (uint8)(~khz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: khz_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return khz_MOD_SRC & khz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: khz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    khz_PHASE = clkPhase & khz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: khz_GetPhase
********************************************************************************
*
* Summary:
...
{
    return khz_PHASE & khz_PHASE_MASK;
}
